...
def load_categories(category_names):
    ...
    for name in category_names:
        categories.append( (name, load_images("./train/" + name + "/*")) )
    return categories
...
def test_recognition(categories, test_samples_per_category):
    ...
    for category, imgs in categories:
        ...
        for i in range(test_samples_per_category):
            ...
            recognized_category, stats = recognizer.recognize_nn(output_img, output_stats=True)
...
category_names = [
#          "headphone",    "menorah",      "sunflower",       "strawberry",         "snoopy",
#           "barrel",       "camera",       "soccer_ball",  
#          "panda",
         "revolver"] 
#           "yin_yang",     "stop_sign",    "pyramid",      "nautilus",     "scissors",
#           "schooner",     "butterfly",    "cup",          "inline_skate", "dollar_bill",
print "loading image files..."
categories = load_categories(category_names)

recognizer = Recognizer(
        categories_images=categories,
        samples_per_image_count=10,
        vocab_size=100, max_images_per_category=10,
        verbose=True)
